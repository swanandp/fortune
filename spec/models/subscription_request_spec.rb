...
RSpec.describe SubscriptionRequest, type: :model do
  ...
  describe "confirm!" do
    ...
    it "creates a subscription object upon confirmation" do
      expect { subscription_request.confirm! }
        .to change { Subscription.where(email: subscription_request.email).count }.from(0).to(1)
    end

    it "deletes itself upon confirmation" do
      expect { subscription_request.confirm! }
        .to change { SubscriptionRequest.where(email: subscription_request.email).count }.by(-1)
    end
  end
end
